...
- 2026-07-11: Amazon product image wiring done, local commit only (`scripts/amazon-image-sync.mjs` prebuild + `amazonImage()` in `affiliate.ts` + `PickCard`/`PriceGrid` render). CF Pages needs `CREATORS_CREDENTIAL_ID`/`CREATORS_CREDENTIAL_SECRET` env vars set before deployed builds show live images; without them the prebuild step warns and skips, keeping today's `src/data/amazon-images.json` (gitignored, not pushed).
